...
class AESFastEngine extends BaseBlockCipher {
  ...
  List<List<int>> _initialWorkingKey;
  int _initialRounds;
  ...
  AESFastEngine(Map<Param, dynamic> params)
      : super("AES", params),
  ...
  void reset() {
    ...
    _workingKey = _initialWorkingKey;
  }
  ...
  void _computeInitialValues() {
    final key = new Uint8List.fromList(_key);

    int KC = (key.length / 4).floor(); // key length in words
    ...
    _initialWorkingKey =
        new List.generate(_initialRounds + 1, (int i) => new List<int>(4)); // 4 words in a block

    // Copy the key into the round key array.
    var keyView = new ByteData.view(key.buffer);
    for (var i = 0,
        t = 0; i < key.length; i += 4, t++) {
      var value = unpack32(keyView, i, Endianness.LITTLE_ENDIAN);
      _initialWorkingKey[t >> 2][t & 3] = value;
    }

    // While not enough round key material calculated calculate new values.
    int k = (_initialRounds + 1) << 2;
    for (int i = KC; i < k; i++) {
      int temp = _initialWorkingKey[(i - 1) >> 2][(i - 1) & 3].toInt();
      if ((i % KC) == 0) {
      ...
      var value = _initialWorkingKey[(i - KC) >> 2][(i - KC) & 3] ^ temp;
      _initialWorkingKey[i >> 2][i & 3] = value;
    }

    if (!_forEncryption) {
      for (var j = 1; j < _initialRounds; j++) {
        for (var i = 0; i < 4; i++) {
          var value = _inv_mcol(_initialWorkingKey[j][i].toInt());
          _initialWorkingKey[j][i] = value;
        }
      }
    }
  }
